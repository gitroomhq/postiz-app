...
export class RedisVotesService {
  ...
  async userVoted(
    ...
    const value = await this._redisService.client().hGet(`vote:${key}:${voteId}:${voteToId}`, userId);
  ...
  async redisVote(
    ...
    if (type === 'single' && (newValue === 1 || newValue === -1 || newValue === 0)) {
      await this._redisService.client().executeIsolated(async (client) => {
        await client.watch(`vote:${key}:${voteId}:${voteToId}`);

        const value = (await client.hGet(`vote:${key}:${voteId}:${voteToId}`, userId) || "0");
    ...
    return this.getRedisTotalVotes(key, voteId, voteToId);
  }
}

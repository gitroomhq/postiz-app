...
export async function postWorkflowV107({
  ...
  // this time, keeping its original releaseURL/releaseId/publishDate
  recycleAt?: string;
}) {
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (const _ of iterate) {
      try {
        ...
        // mark post as successful. Recycles keep the row's original
        // releaseURL/releaseId - the first publication's record must survive
        if (isRecycle) {
          await updatePostKeepRelease(
            postsList[i].id,

...
import { CommentsComponents } from "../../../../../components/preview/comments.components";
import dayjs from 'dayjs';
import utc from 'dayjs/plugin/utc';
import { VideoOrImage } from "../../../../../../../../libraries/react-shared-libraries/src/helpers/video.or.image";
import { CopyClient } from "../../../../../components/preview/copy.client";
import { getT } from "../../../../../../../../libraries/react-shared-libraries/src/translation/get.translation.service.backend";
import dynamicLoad from 'next/dynamic';
const RenderPreviewDate = dynamicLoad(() => import('@gitroom/frontend/components/preview/render.preview.date').then((mod) => mod.RenderPreviewDate), { ssr: false });
dayjs.extend(utc);
...
export default function Auth(_a) {
    return __awaiter(this, arguments, void 0, function* ({ params: { id }, searchParams, }) {
        const post = yield (yield internalFetch(`/public/posts/${id}`)).json();
        const t = yield getT();
        if (!post.length) {
            return (<div className="text-white fixed start-0 top-0 w-full h-full flex justify-center items-center text-[20px]">
      ...
      <div className="mx-auto w-full max-w-[1346px] py-3 text-white">
        <div className="flex items-center justify-between">
              ...
              <div className="min-w-[55px]">
                <Link href="/" className="text-2xl flex items-center justify-center gap-[10px] text-textColor order-1">
                  <div className="max-w-[55px]">
                    <Image src={'/postiz.svg'} width={55} height={55} alt="Logo"/>
                  </div>
                  <div>
          ...
          <div className="text-sm text-gray-400 flex items-center gap-[20px]">
            {!!(searchParams === null || searchParams === void 0 ? void 0 : searchParams.share) && (<div>
                <CopyClient />
              </div>)}
      ...
      <div className="flex flex-col lg:flex-row text-white w-full max-w-[1346px] mx-auto">
        <div className="flex-1">
          <div className="gap-[20px] flex flex-col">
            {post.map((p, index) => (<div key={String(p.id)} className="relative px-4 py-4 bg-third border border-tableBorder">
                <div className="flex space-x-3">
                  <div>
                    <div className="flex shrink-0 rounded-full h-30 w-30 relative">
                      <div className="w-[50px] h-[50px] z-[20]">
                        <img className="w-full h-full relative z-[20] bg-black aspect-square rounded-full border-tableBorder" alt={post[0].integration.name} src={post[0].integration.picture}/>
                      </div>
                      <div className="absolute -end-[5px] -bottom-[5px] w-[30px] h-[30px] z-[20]">
                        <img className="w-full h-full bg-black aspect-square rounded-full border-tableBorder" alt={post[0].integration.providerIdentifier} src={`/icons/platforms/${post[0].integration.providerIdentifier}.png`}/>
                      </div>
                    </div>
                  </div>
                  <div className="flex-1 space-y-1">
                    ...
                    __html: p.content,
                      ...
                      <div className="flex w-full gap-[10px]">
                        {JSON.parse((p === null || p === void 0 ? void 0 : p.image) || '[]').map((p) => (<div key={p.name} className="flex-1 rounded-[10px] max-h-[500px] overflow-hidden">

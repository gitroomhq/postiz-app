...
export const LogoutComponent = ({ isIcon }) => {
    const fetch = useFetch();
    const { isGeneral, isSecured } = useVariables();
    const t = useT();
    const logout = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        if (yield deleteDialog(t('are_you_sure_you_want_to_logout', 'Are you sure you want to logout?'), t('yes_logout', 'Yes logout'))) {
            if (!isSecured) {
                setCookie('auth', '', -10);
            }
            else {
                yield fetch('/user/logout', {
                    method: 'POST',
                });
            }

...
export const MediaBox: FC<{
  ...
  const loadUndeletableIds = useCallback(async () => {
    if (standalone && bulkDeleteMode) {
      const response = await fetch('/media/deletable');
      const data = await response.json();
      setUndeletableIds(data.undeletableIds || []);
    }
  }, [standalone, bulkDeleteMode, fetch]);
  ...
  const selectAllDeletable = useCallback(() => {
    ...
    const deletableIds = data.results
      .filter((m: any) => !undeletableIds.includes(m.id))
      .map((m: any) => m.id);
    setBulkSelected(deletableIds);
  }, [data?.results, undeletableIds]);
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    if (!(await deleteDialog(
      t('bulk_delete_confirm', `Are you sure you want to delete ${bulkSelected.length} media items? This cannot be undone.`)
    ))) {
      return;
    }
    ...
    try {
      const response = await fetch('/media/bulk', {
        ...
        body: JSON.stringify({ mediaIds: bulkSelected }),
      });
      ...
      if (result.skippedIds?.length > 0) {
        alert(t('bulk_delete_skipped', `Deleted ${result.deletedCount} items. ${result.skippedIds.length} items were skipped because they are used in unpublished posts.`));
      }
    } catch (error) {
    ...
    <DropFiles className="flex flex-col flex-1" onDrop={dragAndDrop}>
      <div className="flex flex-col flex-1">
        ...
        >
          ...
          {standalone && !isLoading && !!data?.results?.length && (
            <button
              onClick={() => {
              ...
              className={clsx(
                "cursor-pointer ml-[8px] h-[44px] px-[18px] justify-center items-center rounded-[8px] flex gap-[8px]",
                bulkDeleteMode
                  ? "bg-red-500/20 text-red-400 border border-red-500/50"
                  : "bg-btnSimple changeColor"
              )}
            >
        ...
        {standalone && bulkDeleteMode && !isLoading && !!data?.results?.length && (
          <div className="flex items-center gap-[8px] mt-[8px] p-[12px] bg-red-500/10 rounded-[8px] border border-red-500/30">
            <button
              onClick={selectAllDeletable}
              className="cursor-pointer h-[36px] px-[12px] justify-center items-center rounded-[6px] flex bg-btnSimple changeColor text-[13px]"
            >
            ...
            <div className="flex-1 text-[13px] text-newTextColor/60">
              {bulkSelected.length} selected
              {undeletableIds.length > 0 && ` (${undeletableIds.length} items in unpublished posts)`}
            </div>
            <button
              onClick={handleBulkDelete}
              disabled={bulkSelected.length === 0 || isBulkDeleting}
              className="cursor-pointer h-[36px] px-[16px] justify-center items-center rounded-[6px] flex bg-red-500 text-white disabled:opacity-50 disabled:cursor-not-allowed text-[13px]"
            >
              {isBulkDeleting ? 'Deleting...' : `Delete ${bulkSelected.length} Items`}
            </button>
          </div>
        )}
                ...
                >
                  <div
                    className={clsx(
                      ...
                      !!selected.find((p) => p.id === media.id)
                        ? 'border-[#612BD3]'
                        : bulkDeleteMode && bulkSelected.includes(media.id)
                          ? 'border-red-500'
                          : 'border-transparent',
                      bulkDeleteMode && undeletableIds.includes(media.id) && 'opacity-50'
                    )}
                    onClick={bulkDeleteMode ? () => {
                      if (!undeletableIds.includes(media.id)) {
                        toggleBulkSelect(media.id);
                      }
                  ...
                  >
                    {bulkDeleteMode && (
                      <div
                        className={clsx(
                          "absolute top-[4px] left-[4px] w-[20px] h-[20px] rounded-[4px] flex items-center justify-center z-[50]",
                          undeletableIds.includes(media.id)
                            ? "bg-gray-500/80 cursor-not-allowed"
                            : bulkSelected.includes(media.id)
                              ? "bg-red-500"
                              : "bg-black/50 border border-white/50"
                        )}
                      >
                        {bulkSelected.includes(media.id) && (
                          <svg xmlns="http://www.w3.org/2000/svg" className="w-[14px] h-[14px] text-white" viewBox="0 0 20 20" fill="currentColor">
                    ...
                    {!bulkDeleteMode && !!selected.find((p: any) => p.id === media.id) ? (

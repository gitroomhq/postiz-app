...
const STATUS_COLORS: Record<string, string> = {
  PENDING_APPROVAL: '#e89a7b',
  APPROVED: '#2897bf',
  ADJUSTMENT_REQUESTED: '#cf6295',
  PUBLISHED: '#22c55e',
  DRAFT: '#888',
  ARCHIVED: '#888',
};
...
function ItemCard({ item, token, onAction }: { item: PortalItem; token: string; onAction: () => void }) {
  ...
  return (
    <div
      ...
      style={{
        background: '#fff',
        border: `1px solid ${localStatus === 'APPROVED' ? '#2897bf44' : '#e8e3de'}`,
        boxShadow: '0 2px 12px rgba(0,0,0,0.06)',
      }}
    >
      <div className="flex items-start justify-between gap-[12px] mb-[12px]">
        <h3 className="text-[18px] font-[700] leading-snug" style={{ color: '#201f1d', fontFamily: '"Cormorant Garamond", Georgia, serif' }}>
          {item.title}
        </h3>
        <span
      ...
      {item.body && (
        <p className="text-[14px] leading-relaxed mb-[16px]" style={{ color: '#5a5550' }}>
          {item.body}
        </p>
      )}
        ...
        <div className="mb-[16px] space-y-[8px]">
          {item.events.map((ev) => (
            <div key={ev.id} className="text-[12px] px-[12px] py-[8px] rounded-[8px]" style={{ background: ev.byGuest ? '#fdf5f0' : '#f5f3ff', color: '#5a5550' }}>
              <span className="font-[700]" style={{ color: ev.byGuest ? '#e89a7b' : '#7360aa' }}>
      ...
      {isPending && !requesting && !commenting && (
        <div className="flex flex-wrap gap-[10px]">
          <button
            ...
            style={{ background: '#2897bf', color: '#fff' }}
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-[6px] px-[18px] py-[9px] rounded-full text-[13px] font-[700] transition-all disabled:opacity-50"
            style={{ background: '#cf629518', color: '#cf6295' }}
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-[6px] px-[18px] py-[9px] rounded-full text-[13px] font-[700] transition-all disabled:opacity-50"
            style={{ background: '#f5f3ff', color: '#7360aa' }}
          >
      ...
      {requesting && (
        <div className="mt-[12px]">
          <textarea
            ...
            style={{ border: '1px solid #e8e3de', color: '#201f1d' }}
          />
          <div className="flex gap-[8px] mt-[8px]">
            <button
              ...
              style={{ background: '#cf6295', color: '#fff' }}
            >
              Enviar
            </button>
        ...
        <div className="mt-[12px]">
          ...
          <div className="flex gap-[8px] mt-[8px]">
            <button
              ...
              className="px-[16px] py-[7px] rounded-full text-[12px] font-[700] disabled:opacity-50"
              style={{ background: '#7360aa', color: '#fff' }}
            >
              Comentar
            </button>
      ...
      {localStatus === 'APPROVED' && (
        <div className="flex items-center gap-[6px] text-[13px] font-[700]" style={{ color: '#2897bf' }}>
...
export function PortalApprovalPage({ token }: { token: string }) {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-[24px]" style={{ background: '#faf8f5' }}>
        <div className="text-center max-w-[360px]">
          <AlertCircle size={48} className="mx-auto mb-[16px]" style={{ color: '#cf6295' }} />
          <h1 className="text-[24px] font-[700] mb-[8px]" style={{ fontFamily: '"Cormorant Garamond", Georgia, serif', color: '#201f1d' }}>
            Link inválido
          </h1>
          <p className="text-[14px]" style={{ color: '#5a5550' }}>
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#faf8f5' }}>
        <div className="w-[48px] h-[48px] rounded-full border-[3px] border-t-transparent animate-spin" style={{ borderColor: '#7360aa', borderTopColor: 'transparent' }} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen" style={{ background: '#faf8f5' }}>
      ...
      <div className="px-[24px] py-[32px] max-w-[680px] mx-auto">
        <p className="text-[11px] font-[900] tracking-[0.14em] uppercase mb-[4px]" style={{ color: '#cf6295' }}>
          Vocaccio · Portal de Aprovação
        </p>
        <h1 className="text-[32px] font-[700] leading-tight mb-[6px]" style={{ fontFamily: '"Cormorant Garamond", Georgia, serif', color: '#201f1d' }}>
          Revise o conteúdo
        </h1>
        <p className="text-[14px]" style={{ color: '#5a5550' }}>
        ...
        <div className="flex gap-[16px] mt-[20px]">
          {[
            { icon: Clock, label: 'Aguardando', count: pending.length, color: '#e89a7b' },
            { icon: CheckCircle2, label: 'Aprovados', count: data.items.filter(i => i.status === 'APPROVED').length, color: '#2897bf' },
            { icon: FileText, label: 'Total', count: data.items.length, color: '#7360aa' },
            ...
            <div key={label} className="flex items-center gap-[8px] px-[16px] py-[10px] rounded-[12px]" style={{ background: '#fff', border: '1px solid #e8e3de' }}>
              ...
              <div>
                <p className="text-[18px] font-[800] leading-none" style={{ color: '#201f1d' }}>{count}</p>
                <p className="text-[10px] font-[600] uppercase tracking-wide" style={{ color: '#5a5550' }}>{label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="px-[24px] pb-[48px] max-w-[680px] mx-auto">
        {pending.length > 0 && (
          <>
            <p className="text-[11px] font-[900] tracking-[0.12em] uppercase mb-[12px]" style={{ color: '#e89a7b' }}>
        ...
        {rest.length > 0 && (
          <>
            ...
            <p className="text-[11px] font-[900] tracking-[0.12em] uppercase mb-[12px]" style={{ color: '#5a5550' }}>
              Outros itens
            </p>
            {rest.map((item) => (
              <ItemCard key={item.id} item={item} token={token} onAction={() => mutate()} />
        ...
        {data.items.length === 0 && (
          <div className="text-center py-[64px]">
            ...
            <p className="text-[16px] font-[600]" style={{ color: '#5a5550' }}>Nenhum conteúdo disponível ainda.</p>

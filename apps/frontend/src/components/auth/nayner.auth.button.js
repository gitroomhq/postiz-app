...
export const NeynarAuthButton = (props) => {
    var _a;
    const { children, onLogin } = props;
    const { client_id } = useNeynarContext();
    const [showModal, setShowModal] = useState(false);
    const authWindowRef = useRef(null);
    const neynarLoginUrl = `${(_a = process.env.NEYNAR_LOGIN_URL) !== null && _a !== void 0 ? _a : 'https://app.neynar.com/login'}?client_id=${client_id}`;
    const authOrigin = new URL(neynarLoginUrl).origin;
    const modalRef = useRef(null);
    const handleMessage = useCallback((event) => __awaiter(void 0, void 0, void 0, function* () {
        var _a;
        if (event.origin === authOrigin &&
            event.data &&
            event.data.is_authenticated) {
            (_a = authWindowRef.current) === null || _a === void 0 ? void 0 : _a.close();
            window.removeEventListener('message', handleMessage); // Remove listener here
            delete event.data.user.profile;
            const _user = Object.assign({ signer_uuid: event.data.signer_uuid }, event.data.user);
    ...
    const handleSignIn = useCallback(() => {
        ...
        authWindowRef.current = window.open(neynarLoginUrl, '_blank', windowFeatures);
...
//# sourceMappingURL=nayner.auth.button.js.map
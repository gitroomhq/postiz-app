...
export const Filters = () => {
    ...
    const setToday = useCallback(() => {
        ...
        const currentRange = getDateRange(calendar.display);
        // Check if we're already showing today's range
        if (calendar.startDate === currentRange.startDate &&
    ...
    const next = useCallback(() => {
        ...
        const range = getDateRange(calendar.display, nextStart.format('YYYY-MM-DD'));
        calendar.setFilters({
            startDate: range.startDate,
            endDate: range.endDate,
            display: calendar.display,
    ...
    const previous = useCallback(() => {
        ...
        const range = getDateRange(calendar.display, prevStart.format('YYYY-MM-DD'));
        calendar.setFilters({
            startDate: range.startDate,
            endDate: range.endDate,
            display: calendar.display,
    ...
    const setCurrent = useCallback((type) => () => {
        if (type === 'day') {
            setDay();
        }
        else if (type === 'week') {
    ...
    return (<div className="text-textColor flex flex-col md:flex-row gap-[8px] items-center select-none">
      {!isListView && (<div className="flex flex-grow flex-row items-center gap-[10px]">
          <div className="border h-[42px] border-newTableBorder bg-newTableBorder gap-[1px] flex items-center rounded-[8px] overflow-hidden">
            <div onClick={previous} className="cursor-pointer text-textColor rtl:rotate-180 px-[9px] bg-newBgColorInner h-full flex items-center justify-center hover:text-textItemFocused hover:bg-boxFocused">
            ...
            <div onClick={next} className="cursor-pointer text-textColor rtl:rotate-180 px-[9px] bg-newBgColorInner h-full flex items-center justify-center hover:text-textItemFocused hover:bg-boxFocused">
              <svg xmlns="http://www.w3.org/2000/svg" width="8" height="12" viewBox="0 0 8 12" fill="none">
          ...
          <div className="flex-1 text-[14px] font-[500]">
            <div className="text-center flex h-[42px]">
              <div onClick={setToday} className="hover:text-textItemFocused hover:bg-boxFocused py-[3px] px-[9px] flex justify-center items-center rounded-[8px] transition-all cursor-pointer text-[14px] bg-newBgColorInner border border-newTableBorder">
                {t('today', 'Today')}
              </div>
            </div>
          </div>
      ...
      {isListView && (<div className="flex flex-grow flex-row items-center gap-[10px]">
          <div className="border h-[42px] border-newTableBorder bg-newTableBorder gap-[1px] flex items-center rounded-[8px] overflow-hidden">
            <div onClick={previousPage} className={clsx('text-textColor rtl:rotate-180 px-[9px] bg-newBgColorInner h-full flex items-center justify-center', calendar.listPage > 0
            ...
            <div onClick={nextPage} className={clsx('text-textColor rtl:rotate-180 px-[9px] bg-newBgColorInner h-full flex items-center justify-center', calendar.listPage < calendar.listTotalPages - 1
                ? 'cursor-pointer hover:text-textItemFocused hover:bg-boxFocused'
                : 'opacity-50 cursor-not-allowed')}>
              <svg xmlns="http://www.w3.org/2000/svg" width="8" height="12" viewBox="0 0 8 12" fill="none">
                <path d="M1.5 11L6.5 6L1.5 1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
      ...
      <SelectCustomer customer={calendar.customer} onChange={(customer) => setCustomer(customer)} integrations={calendar.integrations}/>
      {!isListView && (<div className="flex flex-row p-[4px] border border-newTableBorder rounded-[8px] text-[14px] font-[500]">
          <div className={clsx('pt-[6px] pb-[5px] cursor-pointer w-[74px] text-center rounded-[6px]', calendar.display === 'day' && 'text-textItemFocused bg-boxFocused')} onClick={setDay}>
            {t('day', 'Day')}
          </div>
          ...
          <div className={clsx('pt-[6px] pb-[5px] cursor-pointer w-[74px] text-center rounded-[6px]', calendar.display === 'month' && 'text-textItemFocused bg-boxFocused')} onClick={setMonth}>
            {t('month', 'Month')}
          </div>
        </div>)}
      <div className="flex flex-row p-[4px] border border-newTableBorder rounded-[8px] text-[14px] font-[500]">
        <div onClick={setCalendarView} className={clsx('pt-[6px] pb-[5px] cursor-pointer flex justify-center items-center w-[34px] text-center rounded-[6px]', !isListView && 'text-textItemFocused bg-boxFocused')}>
          {/*calendar*/}
          <svg xmlns="http://www.w3.org/2000/svg" width="17" height="19" viewBox="0 0 17 19" fill="none">
            <path d="M15.75 7.41667H0.75M11.5833 0.75V4.08333M4.91667 0.75V4.08333M4.75 17.4167H11.75C13.1501 17.4167 13.8502 17.4167 14.385 17.1442C14.8554 16.9045 15.2378 16.522 15.4775 16.0516C15.75 15.5169 15.75 14.8168 15.75 13.4167V6.41667C15.75 5.01654 15.75 4.31647 15.4775 3.78169C15.2378 3.31129 14.8554 2.92883 14.385 2.68915C13.8502 2.41667 13.1501 2.41667 11.75 2.41667H4.75C3.34987 2.41667 2.6498 2.41667 2.11502 2.68915C1.64462 2.92883 1.26217 3.31129 1.02248 3.78169C0.75 4.31647 0.75 5.01654 0.75 6.41667V13.4167C0.75 14.8168 0.75 15.5169 1.02248 16.0516C1.26217 16.522 1.64462 16.9045 2.11502 17.1442C2.6498 17.4167 3.34987 17.4167 4.75 17.4167Z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
        </div>
        <div onClick={setList} className={clsx('pt-[6px] pb-[5px] flex justify-center items-center cursor-pointer w-[34px] text-center rounded-[6px]', isListView && 'text-textItemFocused bg-boxFocused')}>
          {/*list*/}

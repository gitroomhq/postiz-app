...
import { VideoWrapper } from "../videos/video.render.component";
import { FormProvider, useForm } from 'react-hook-form';
import { useUser } from "../layout/user.context";
import { VideoContextWrapper } from "../videos/video.context.wrapper";
...
export const Modal = (props) => {
    ...
    return (<VideoContextWrapper.Provider value={{ value: value }}>
      <form onSubmit={form.handleSubmit(generate)} className="flex flex-col gap-[10px]">
        <FormProvider {...form}>
                  ...
                  <div className="flex-1">
                    <TopTitle title={'Video Type'}>
                      <div className="mr-[25px]">
                        {(data === null || data === void 0 ? void 0 : data.credits) || 0} credits left
                      </div>
                    </TopTitle>
                  </div>
                  <button onClick={props.close} className="outline-none absolute end-[10px] top-[10px] mantine-UnstyledButton-root mantine-ActionIcon-root bg-primary hover:bg-tableBorder cursor-pointer mantine-Modal-close mantine-1dcetaa" type="button">
                  ...
                  <div className="absolute left-0 top-0 w-full h-full overflow-hidden overflow-y-auto">
                    <div className="mt-[10px] flex w-full justify-center items-center gap-[10px]">
                      <div className="flex-1 flex">
                        <Button className="!flex-1" onClick={() => setPosition('vertical')} secondary={position === 'horizontal'}>
                          Vertical (Stories, Reels)
                        </Button>
                      </div>
                      <div className="flex-1 flex mt-[10px]">
                        <Button className="!flex-1" onClick={() => setPosition('horizontal')} secondary={position === 'vertical'}>
...
export const AiVideo = (props) => {
    ...
    const loadVideoList = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        return (yield (yield fetch('/media/video-options')).json()).filter((f) => f.placement === 'text-to-image');
    }), []);
    ...
    const generateVideo = useCallback((type) => () => __awaiter(void 0, void 0, void 0, function* () {
        setType(type);
        setModal(true);
    }), [value, onChange]);
    if (isLoading || (data === null || data === void 0 ? void 0 : data.length) === 0) {
        return null;
    }
    return (<>
      {modal && (<Modal onChange={onChange} setLoading={setLoading} close={() => {
                setModal(false);
                setType(null);
      ...
      <div className="relative group">
        ...
        ? {
            ...
            'data-tooltip-content': 'Please add at least 30 characters to generate AI video',
        }
        : {})} className={clsx('cursor-pointer h-[30px] rounded-[6px] justify-center items-center flex bg-newColColor px-[8px]', value.length < 30 && 'opacity-50')}>
          {loading && (<div className="absolute start-[50%] -translate-x-[50%]">
              <Loading height={30} width={30} type="spin" color="#fff"/>
            </div>)}
          <div className={clsx('flex gap-[5px] items-center', loading && 'invisible')}>
            <div>
        ...
        {value.length >= 30 && !loading && (<div className="text-[12px] -mt-[10px] w-[200px] absolute bottom-[100%] z-[500] start-0 hidden group-hover:block">
            <ul className="cursor-pointer rounded-[4px] border border-dashed border-newBgLineColor bg-newColColor mt-[3px] p-[5px]">
              {data.map((p) => (<li onClick={generateVideo(p)} key={p.identifier} className="hover:bg-sixth">
                  {p.title}
                </li>))}
            </ul>
          </div>)}
...
//# sourceMappingURL=ai.video.js.map
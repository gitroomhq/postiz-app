...
import { useSearch } from '../layout/search.component';
...
export const AddProviderComponent: FC<{
  ...
  const { update, social, article, invite, onboarding, isMobile } = props;

  const { searchedResults, SearchInputBox } = useSearch(
    social.filter((item) => {
      if (!props.invite) {
        return true;
      }
...
export const FilteredProviderComponent: FC<{
  searchedResults: Social[];
  invite: boolean;
  onboarding?: boolean;
  isMobile?: boolean;
}> = (props) => {
  const { searchedResults, onboarding, isMobile, invite } =
    props;
  ...
  return searchedResults.map((item) => (
    <div
      key={item.identifier}
    ...
    >
      ...
      <div
        className={clsx(isMobile ? '' : 'whitespace-pre-wrap', 'text-center')}
      >
        {item.name}

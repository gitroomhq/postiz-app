...
export const AiImage = (props) => {
    const t = useT();
    const { value, onChange } = props;
    const [loading, setLoading] = useState(false);
    const setLocked = useLaunchStore((p) => p.setLocked);
    const fetch = useFetch();
    const generateImage = useCallback((type) => () => __awaiter(void 0, void 0, void 0, function* () {
        setLoading(true);
        setLocked(true);
        const image = yield (yield fetch('/media/generate-image-with-prompt', {
            method: 'POST',
            body: JSON.stringify({
                prompt: `
<!-- description -->
    ...
    return (<div className="relative group">
      <div {...(value.length < 30
        ? {
            ...
            'data-tooltip-content': 'Please add at least 30 characters to generate AI image',
        }
      ...
      {value.length >= 30 && !loading && (<div className="text-[12px] -mt-[10px] w-[200px] absolute bottom-[100%] z-[500] start-0 hidden group-hover:block">
          <ul className="cursor-pointer rounded-[4px] border border-dashed mt-[3px] p-[5px] border-newBgLineColor bg-newColColor">
            {list.map((p) => (<li onClick={generateImage(p)} key={p} className="hover:bg-sixth">
                {p}
              </li>))}
          </ul>
        </div>)}

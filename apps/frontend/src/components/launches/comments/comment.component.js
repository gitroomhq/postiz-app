...
export const CommentBox = (props) => {
    ...
    const newCommentFunc = useCallback((event) => {
    ...
    return (<div className={clsx('flex', type === 'textarea' ? 'flex-col' : 'flex-row flex items-end gap-[10px]')}>
      <div className={clsx(type === 'input' && 'flex-1')}>
        <Component label={type === 'textarea' ? 'Add comment' : ''} placeholder={type === 'input' ? 'Add comment' : ''} name="comment" disableForm={true} value={newComment} onChange={newCommentFunc}/>
      </div>
      <Button disabled={newComment.length < 2} onClick={changeIt} className={clsx(type === 'input' && 'mb-[27px]')}>
        {value ? 'Update' : 'Add comment'}
      </Button>
    </div>);
};
export const EditableCommentComponent = (props) => {
    const { comment, onEdit, onDelete } = props;
    const [commentContent, setCommentContent] = useState(comment.content);
    const [editMode, setEditMode] = useState(false);
    const user = useUser();
    const updateComment = useCallback((commentValue) => {
    ...
    const deleteCommentFunction = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        if (yield deleteDialog('Are you sure you want to delete this comment?', 'Yes, Delete')) {
            onDelete();
        }
    }), []);
    if (editMode) {
        return (<CommentBox type="input" value={commentContent} onChange={updateComment}/>);
    }
...
export const CommentComponent = (props) => {
    ...
    const load = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        const data = yield (yield fetch(`/comments/${date.utc().format('YYYY-MM-DDTHH:mm:00')}`)).json();
        setCommentsList(data);
    }), []);
    useEffect(() => {
        load();
    }, []);
    const editComment = useCallback((comment) => (content) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const addComment = useCallback((content) => __awaiter(void 0, void 0, void 0, function* () {
        const { id } = yield (yield fetch('/comments', {
            method: 'POST',
            body: JSON.stringify({
                content,
                date: date.utc().format('YYYY-MM-DDTHH:mm:00'),
    ...
    const deleteComment = useCallback((comment) => () => __awaiter(void 0, void 0, void 0, function* () {
        yield fetch(`/comments/${comment.id}`, {
            method: 'DELETE',
        });
        setCommentsList((list) => list.filter((item) => item.id !== comment.id));
    }), [commentsList, setCommentsList]);
    const deleteChildrenComment = useCallback((parent, children) => () => __awaiter(void 0, void 0, void 0, function* () {
        yield fetch(`/comments/${children.id}`, {
            method: 'DELETE',
        });
        setCommentsList((list) => list.map((item) => {
            if (item.id === parent.id) {
                return Object.assign(Object.assign({}, item), { childrenComment: item.childrenComment.filter((child) => child.id !== children.id) });
            }
            return item;
        }));
    }), [commentsList, setCommentsList]);
    const addChildrenComment = useCallback((comment) => (content) => __awaiter(void 0, void 0, void 0, function* () {
        const { id } = yield (yield fetch(`/comments/${comment.id}`, {
            method: 'POST',
            body: JSON.stringify({
                content,
                date: date.utc().format('YYYY-MM-DDTHH:mm:00'),
        ...
        setCommentsList((list) => list.map((item) => {
            if (item.id === comment.id) {
                return Object.assign(Object.assign({}, item), { childrenComment: [
    ...
    const extractNameFromEmailAndCapitalize = useCallback((email) => {
        return (email.split('@')[0].charAt(0).toUpperCase() + email.split('@')[0].slice(1));
    }, []);
    return (<div className="relative flex gap-[20px] flex-col flex-1 rounded-[4px] border border-customColor6 bg-sixth p-[16px] pt-0">
      <TopTitle title={`Comments for ${date.format('DD/MM/YYYY HH:mm')}`}/>
      <button onClick={closeAll} className="outline-none absolute end-[20px] top-[15px] mantine-UnstyledButton-root mantine-ActionIcon-root hover:bg-tableBorder cursor-pointer mantine-Modal-close mantine-1dcetaa" type="button">
      ...
      <div>
        {commentsList.map((comment, index) => {
            var _a, _b;
            return (<>
            <div key={`comment_${index}_${comment.content}`} className={clsx(`flex relative flex-col`, ((_a = comment === null || comment === void 0 ? void 0 : comment.childrenComment) === null || _a === void 0 ? void 0 : _a.length) && 'gap-[10px]')}>
              <div className="flex gap-[8px]">
                <div className="w-[40px] flex flex-col items-center">
                  <div className={`rounded-full relative z-[2] text-blue-500 font-bold flex justify-center items-center w-[40px] h-[40px] bg-white border-tableBorder border`}>
                ...
                <div className="flex-1 flex flex-col gap-[4px]">
                  ...
                  <EditableCommentComponent onDelete={deleteComment(comment)} onEdit={editComment(comment)} comment={comment}/>
                </div>
              </div>

              <div className="flex flex-col gap-[10px]">
                {(_b = comment === null || comment === void 0 ? void 0 : comment.childrenComment) === null || _b === void 0 ? void 0 : _b.map((childComment, index2) => (<div key={`comment2_${index2}_${childComment.content}`} className={clsx(`flex gap-[8px] relative`)}>
                    <div className="w-[40px] flex flex-col items-center">
                      <div className={`rounded-full relative z-[2] text-blue-500 font-bold flex justify-center items-center w-[40px] h-[40px] bg-white border-tableBorder border`}>
                    ...
                    <div className="flex-1 flex flex-col gap-[4px]">
                      ...
                      <EditableCommentComponent onDelete={deleteChildrenComment(comment, childComment)} onEdit={editComment(childComment)} comment={childComment}/>
                    </div>
                  </div>))}
              </div>
            </div>
            <div className="flex">
              ...
              <div className="flex-1">
                <CommentBox type="input" onChange={addChildrenComment(comment)}/>
              </div>
            </div>
          </>);
        })}

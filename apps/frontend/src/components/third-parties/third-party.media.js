...
import { thirdPartyList } from "./third-party.wrapper";
...
export const ThirdPartyPopup = (props) => {
    ...
    const Component = useMemo(() => {
        ...
        return (((_a = thirdPartyList.find((p) => p.identifier === thirdParty.identifier)) === null || _a === void 0 ? void 0 : _a.Component) || EmptyComponent);
    }, [thirdParty]);
    ...
    useEffect(() => {
        var _a;
        (_a = refNew === null || refNew === void 0 ? void 0 : refNew.current) === null || _a === void 0 ? void 0 : _a.scrollIntoView({
            behavior: 'smooth',
        });
    }, []);
    return (<div className={clsx('flex flex-wrap flex-col gap-[10px] pt-[20px]')}>
      {!thirdParty && (<div className="grid grid-cols-4 gap-[10px] justify-items-center justify-center">
          {thirdParties.map((p) => (<div onClick={() => {
                    setThirdParty(p);
                }} key={p.identifier} className="w-full h-full p-[20px] min-h-[100px] text-[14px] bg-third hover:bg-input transition-all text-textColor relative flex flex-col gap-[15px] cursor-pointer">
              <div>
      ...
      {thirdParty && (<>
          <div>
            <div className="cursor-pointer float-left" onClick={() => setThirdParty(null)}>
              {'<'} Back
            </div>
          </div>
          <ThirdPartyContext.Provider value={Object.assign(Object.assign({}, thirdParty), { data: allData, close, onChange })}>
            <Component />
          </ThirdPartyContext.Provider>
        </>)}
    </div>);
};
export const ThirdPartyMedia = (props) => {
    const { allData, onChange } = props;
    const t = useT();
    const fetch = useFetch();
    const modals = useModals();
    const thirdParties = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        return (yield (yield fetch('/third-party')).json()).filter((f) => f.position === 'media');
    }), []);
...
//# sourceMappingURL=third-party.media.js.map
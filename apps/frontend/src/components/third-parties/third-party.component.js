...
import { ThirdPartyListComponent } from "./third-party.list.component";
...
export const ThirdPartyMenuComponent = (props) => {
    ...
    const deleteChannel = (id) => () => __awaiter(void 0, void 0, void 0, function* () {
        setShow(false);
        if (!(yield deleteDialog('Are you sure you want to delete this integration?'))) {
            return;
        }
        const res = yield fetch(`/third-party/${id}`, {
            method: 'DELETE',
        });
        if (res.ok) {
...
export const ThirdPartyComponent = () => {
    const t = useT();
    const fetch = useFetch();
    const integrations = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        return (yield fetch('/third-party')).json();
    }), []);
          ...
          <div className="flex flex-col gap-[10px]">
            <div className="flex-1 flex flex-col gap-[14px]">
              <div className={clsx('gap-[16px] flex flex-col relative justify-center group/profile hover:bg-boxHover rounded-e-[8px]')}>
                {!isLoading && !(data === null || data === void 0 ? void 0 : data.length) ? (<div>No Integrations Yet</div>) : (data === null || data === void 0 ? void 0 : data.map((p) => (<div key={p.id} className={clsx('flex gap-[8px] items-center')}>
                      <div className="h-full w-[4px] -ms-[12px] rounded-s-[3px] opacity-0 group-hover/profile:opacity-100 transition-opacity">
                        <SVGLine />
                      </div>
                      <div className={clsx('relative rounded-full flex justify-center items-center bg-fifth')} data-tooltip-id="tooltip" data-tooltip-content={p.title}>
                        <ImageWithFallback fallbackSrc={`/icons/third-party/${p.identifier}.png`} src={`/icons/third-party/${p.identifier}.png`} className="rounded-full" alt={p.title} width={32} height={32}/>
                      </div>
                      <div 
        // @ts-ignore
        role="Handle" className={clsx('flex-1 whitespace-nowrap text-ellipsis overflow-hidden group-[.sidebar]:hidden')} data-tooltip-id="tooltip" data-tooltip-content={p.title}>
                        {p.name}
                      </div>
...
//# sourceMappingURL=third-party.component.js.map
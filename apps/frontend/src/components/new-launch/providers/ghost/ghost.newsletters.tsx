...
export const GhostNewsletters: FC<{
  name: string;
  label: string;
  onChange: (event: {
    target: {
  ...
  const [newsletters, setNewsletters] = useState<GhostNewsletter[]>([]);
  const [selectedValue, setSelectedValue] = useState<string>('');
  const [isLoading, setIsLoading] = useState(true);

  useEffect(() => {
    // Fetch available newsletters from Ghost
    customFunc.get('newsletters').then((data: GhostNewsletter[]) => {
      setNewsletters(data || []);
      setIsLoading(false);
  ...
  return (
    <div>
      ...
      <select
        ...
        className="w-full bg-[#1a1a2e] border border-white/20 rounded-[4px] px-[12px] py-[8px] text-white text-[14px]"
      >
        <option value="">Default newsletter</option>
        {newsletters.map((nl) => (
          <option key={nl.value} value={nl.value}>
            {nl.label}
          </option>
        ))}
      </select>

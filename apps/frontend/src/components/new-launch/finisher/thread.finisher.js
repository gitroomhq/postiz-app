...
export const ThreadFinisher = () => {
    ...
    register('thread_finisher', {
        value: t('that_a_wrap', {
            username: ((_a = integration.integration) === null || _a === void 0 ? void 0 : _a.display) || ((_b = integration.integration) === null || _b === void 0 ? void 0 : _b.name),
        }),
    });
    const slider = watch('active_thread_finisher');
    const value = watch('thread_finisher');
    return (<div className="flex flex-col gap-[10px] border-tableBorder border p-[15px] rounded-lg mb-5">
      <div className="flex items-center">
        <div className="flex-1">Add a thread finisher</div>
        <div>
          <Slider value={slider ? 'on' : 'off'} onChange={(p) => setValue('active_thread_finisher', p === 'on')} fill={true}/>
        </div>
      </div>
      <div className="w-full mt-[20px]">
        <div className={clsx(!slider && 'relative opacity-25 pointer-events-none editor')}>
          <div>
            <div className="flex gap-[4px]">
              <div className="flex-1 editor text-textColor">
                <Editor comments={true} chars={{}} selectedIntegration={[]} onChange={(val) => setValue('thread_finisher', val)} value={value} totalPosts={1} dummy={dummy}/>
              </div>
            </div>
          </div>
        </div>
...
//# sourceMappingURL=thread.finisher.js.map
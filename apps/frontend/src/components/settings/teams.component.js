...
export const AddMember = () => {
    ...
    const submit = useCallback((values) => __awaiter(void 0, void 0, void 0, function* () {
        const { url } = yield (yield fetch('/settings/team', {
            method: 'POST',
...
export const TeamsComponent = () => {
    ...
    const modals = useModals();
    const t = useT();
    const myLevel = (user === null || user === void 0 ? void 0 : user.role) === 'USER' ? 0 : (user === null || user === void 0 ? void 0 : user.role) === 'ADMIN' ? 1 : 2;
    const getLevel = useCallback((role) => role === 'USER' ? 0 : role === 'ADMIN' ? 1 : 2, []);
    const loadTeam = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        return (yield (yield fetch('/settings/team')).json()).users;
    }), []);
    ...
    const remove = useCallback((toRemove) => () => __awaiter(void 0, void 0, void 0, function* () {
        if (!(yield deleteDialog(t('are_you_sure_remove_team_member', 'Are you sure you want to remove this team member?')))) {
            return;
        }
        yield fetch(`/settings/team/${toRemove.user.id}`, {
            method: 'DELETE',
        });
        yield mutate();
    }), [t]);
    return (<div className="flex flex-col">
      ...
      <div className="my-[16px] mt-[16px] bg-sixth border-fifth border rounded-[4px] p-[24px] flex flex-col gap-[24px]">
        <div className="flex flex-col gap-[16px]">
          {(data || []).map((p) => (<div key={p.user.id} className="flex items-center">

...
import { isUSCitizen } from "../launches/helpers/isuscitizen.utils";
const dateMetrics = [
...
const MetricComponent = () => {
    ...
    const [timezone, setTimezone] = useState(localStorage.getItem('timezone') || dayjs.tz.guess());
    ...
    return (<div className="my-[16px] mt-[16px] bg-sixth border-fifth border rounded-[4px] p-[24px] flex flex-col gap-[24px]">
      ...
      <Select name="metric" disableForm={true} label="" onChange={changeMetric}>
        {dateMetrics.map((metric) => (<option key={metric.value} value={metric.value} selected={currentMetric === (metric.value === 'US')}>
            {metric.label}
          </option>))}
      </Select>

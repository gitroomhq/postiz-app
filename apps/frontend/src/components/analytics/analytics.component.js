...
import { StarsAndForks } from "./stars.and.forks";
import { useCallback } from 'react';
import { StarsTableComponent } from "./stars.table.component";
...
export const AnalyticsComponent = () => {
    ...
    const { isLoading: isLoadingAnalytics, data: analytics } = useSWR('/analytics', load);
    const { isLoading: isLoadingTrending, data: trending } = useSWR('/analytics/trending', load);
    if (isLoadingAnalytics || isLoadingTrending) {
        return <LoadingComponent />;
    }
    return (<div className="flex gap-[24px] flex-1">

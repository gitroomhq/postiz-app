...
function mergeDataPoints(data, numPoints) {
    const res = chunk(data, Math.ceil(data.length / numPoints));
    return res.map((row) => {
        var _a;
        return {
            date: `${row[0].date} - ${(_a = row === null || row === void 0 ? void 0 : row.at(-1)) === null || _a === void 0 ? void 0 : _a.date}`,
            total: row.reduce((acc, curr) => acc + curr.total, 0),
        };
    });
}
export const ChartSocial = (props) => {
...
//# sourceMappingURL=chart-social.js.map
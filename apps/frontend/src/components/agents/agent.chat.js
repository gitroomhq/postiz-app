...
import { MediaPortal, PropertiesContext, } from "./agent";
...
import { ExistingDataContextProvider } from "../launches/helpers/use.existing.data";
...
export const AgentChat = () => {
    ...
    const t = useT();
    return (<CopilotKit {...(params.id === 'new' ? {} : { threadId: params.id })} credentials="include" runtimeUrl={backendUrl + '/copilot/agent'} showDevConsole={false} agent="postiz" properties={{
      ...
      <div style={{
        ...
        <div className="absolute left-0 w-full h-full pb-[20px]">
          <CopilotChat className="w-full h-full" labels={{
...
`),
        }} UserMessage={Message} Input={NewInput}/>
        </div>
      </div>
    </CopilotKit>);
};
const LoadMessages = ({ id }) => {
    ...
    const loadMessages = useCallback((idToSet) => __awaiter(void 0, void 0, void 0, function* () {
        const data = yield (yield fetch(`/copilot/${idToSet}/list`)).json();
        setMessages(data.uiMessages.map((p) => {
...
const Message = (props) => {
    ...
    const convertContentToImagesAndVideo = useMemo(() => {
        var _a;
        return (((_a = props.message) === null || _a === void 0 ? void 0 : _a.content) || '')
    ...
    }, [(_a = props.message) === null || _a === void 0 ? void 0 : _a.content]);
    return (<div className="copilotKitMessage copilotKitUserMessage min-w-[300px]" dangerouslySetInnerHTML={{ __html: convertContentToImagesAndVideo }}/>);
};
const NewInput = (props) => {
    ...
    return (<>
      ...
      <Input {...props} onChange={setValue} onSend={(text) => {
            const send = props.onSend(text +
                (media.length > 0
                    ? '\n[--Media--]' +
                        media
                            .map((m) => m.path.indexOf('mp4') > -1
                            ? `Video: ${m.path}`
                            : `Image: ${m.path}`)
                            .join('\n') +
                        '\n[--Media--]'
...
${properties.length
                    ? `[--integrations--]
Use the following social media platforms: ${JSON.stringify(properties.map((p) => ({
                        id: p.id,
...
const OpenModal = ({ args, respond }) => {
    const modals = useModals();
    const { properties } = useContext(PropertiesContext);
    const startModal = useCallback(() => __awaiter(void 0, void 0, void 0, function* () {
        for (const integration of args.list) {
            yield new Promise((res) => {
                ...
                modals.openModal({
                    ...
                    children: (<ExistingDataContextProvider value={{
                            group,
                            integration: integration.integrationId,
                            integrationPicture: properties.find((p) => p.id === integration.integrationId)
                                .picture || '',
              ...
              <AddEditModal date={dayjs.utc(integration.date)} allIntegrations={properties} integrations={properties.filter((p) => p.id === integration.integrationId)} onlyValues={integration.posts.map((p) => ({
                            content: p.content,
                            id: makeId(10),
                            settings: integration.settings || {},
                            image: p.attachments.map((a) => ({
    ...
    return (<div onClick={() => respond('continue')}>

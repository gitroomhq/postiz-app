...
const useFaqList = () => {
  ...
  return [
    ...
    },
    {
      ...
      description: `${isGeneral ? 'Postiz' : 'Gitroom'} allows you to schedule your posts between different channels. A channel is a publishing platform where you can schedule your posts. For example, you can schedule your posts on Twitter, Linkedin, DEV and Hashnode.`,
    },
    {
...
export const FAQSection: FC<{ title: string; description: string }> = (props) => {
  ...
  return (
    <div
      className="bg-sixth p-4 border border-tableBorder rounded-md flex flex-col cursor-pointer"
      onClick={changeShow}
    >
      <div className={`text-lg ${interClass} flex justify-between items-center`}>
      ...
      <div className={clsx('transition-all duration-500 overflow-hidden', !show ? 'max-h-0' : 'max-h-96')}>
        <pre
          onClick={(e) => {
            e.stopPropagation();
          }}
          className={`mt-4 w-full text-wrap ${interClass} font-normal text-base text-customColor17 select-text`}
...
export const FAQComponent: FC = () => {
  ...
  return (
    <div className="mx-auto max-w-4xl p-4">
      <h3 className="text-2xl text-center mt-20 mb-10">Frequently Asked Questions</h3>
      <div className="gap-6 flex flex-col select-none">
        {list.map((item, index) => (

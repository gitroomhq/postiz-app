...
export class PostsController {
  ...
  @Post('/')
  @CheckPolicies([AuthorizationActions.Create, Sections.POSTS_PER_MONTH], [AuthorizationActions.Create, Sections.ADMIN])
  async createPost(
    @GetOrgFromRequest() org: Organization,
    @GetUserFromRequest() user: User,
    @Body() rawBody: any
    ...
    return this._postsService.createPost(
      ...
      false,
      approvalStatus,
      role === 'SUPERADMIN' ? user.id : undefined, // approvedBy - only SUPERADMIN
      user.id                                      // createdBy - could be SUPERADMIN or ADMIN only
    );
  }
  ...
  @Delete('/:group')
  @CheckPolicies([AuthorizationActions.Delete, Sections.APPROVE_POST])
  deletePost(
    @GetOrgFromRequest() org: Organization,
    @Param('group') group: string
  ) {
  ...
  async getPendingApproval(@GetOrgFromRequest() org: Organization) {
    return this._postsService.getPendingApproval(org.id);
  }

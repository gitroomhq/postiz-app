...
export const CustomSelect = (props) => {
    const { options, onChange, placeholder, className, removeError, label, translationKey, translationParams } = props, rest = __rest(props, ["options", "onChange", "placeholder", "className", "removeError", "label", "translationKey", "translationParams"]);
    ...
    const err = useMemo(() => {
        ...
        let errIn = (_a = form === null || form === void 0 ? void 0 : form.formState) === null || _a === void 0 ? void 0 : _a.errors;
        for (let i = 0; i < split.length; i++) {
            // @ts-ignore
            errIn = errIn === null || errIn === void 0 ? void 0 : errIn[split[i]];
        }
        return errIn === null || errIn === void 0 ? void 0 : errIn.message;
    }, [props.name, form]);
    const option = useMemo(() => {
        if ((value === null || value === void 0 ? void 0 : value.value) && options.length) {
    ...
    const setOption = useCallback((newOption) => (e) => {
    ...
    return (<div className={clsx('flex flex-col gap-[6px] relative', className)}>
      ...
      <div className={clsx('bg-input h-[44px] border-fifth border rounded-[4px] text-inputText placeholder-inputText items-center justify-center flex')} onClick={changeOpen}>
        <div className="flex-1 ps-[16px] text-[14px] select-none flex gap-[8px]">
          {!!option.icon && (<div className="flex justify-center items-center">
              {option.icon}
            </div>)}
        ...
        <div className="pe-[16px] flex gap-[8px]">
          ...
          {!!value && (<div onClick={setOption(undefined)}>
              <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
      ...
      {isOpen && (<div className={clsx(label && !removeError && '-mt-[23px]', 'z-[100] absolute w-full top-[100%] start-0 flex items-center rounded-bl-[4px] rounded-br-[4px] flex-col bg-fifth gap-[1px] border-l border-r border-b border-fifth overflow-hidden')}>
          {options.map((option) => (<div key={option.value} onClick={setOption(option)} className="px-[16px] py-[8px] bg-input w-full flex gap-[8px] hover:bg-customColor3 select-none cursor-pointer">
              {!!option.icon && (<div className="flex justify-center items-center">
                  {option.icon}
                </div>)}
      ...
      {!removeError && (<div className="text-red-400 text-[12px]">
          {err || <>&nbsp;</>}
        </div>)}
    </div>);
};

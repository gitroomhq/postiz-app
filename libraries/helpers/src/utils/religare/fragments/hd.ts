// Human Design fragment bank — Type/Authority/Profile/Definition narratives,
// tagged with the shared theme vocabulary. Pure (no ephemeris) — the calc
// engine that feeds these lives in ../hd.ts.

import { Fragment, HDAuthority, HDDefinition, HDType, ThemeKey } from '../types';

const TYPE_TEXT: Record<HDType, { text: string; tags: Partial<Record<ThemeKey, number>> }> = {
  generator: {
...
const AUTHORITY_TEXT: Record<HDAuthority, { text: string; tags: Partial<Record<ThemeKey, number>> }> = {
  emotional: {
...
const DEFINITION_TEXT: Record<HDDefinition, { text: string; tags: Partial<Record<ThemeKey, number>> }> = {
  none: {
...
const PROFILE_LABEL_PT: Record<string, string> = {
...
const PROFILE_TAGS: Record<string, Partial<Record<ThemeKey, number>>> = {
...
export function definitionFragment(definition: HDDefinition): Fragment {

...
export class LoadToolsService {
  ...
  async agent() {
    ...
    return new Agent({
      ...
      - To find or inspect existing posts, use postsListTool with a UTC start and end date - it returns the posts scheduled in that window, one page at a time ("total" and "hasMore" tell if there is more, pass "page" + 1 for the next page). Prefer a narrow window, and for many posts summarise instead of fetching every page.

...
**Analytics live follow-ups.** All-channels Top N heading matches the cards shown (`top_post` / `top_n_posts`); a single channel still says Top 5 posts. Chart tooltips stay fully visible below the app header (`z-220`, `minTop`). Published analytics menus no longer include Preview Post. `/p/:id` share no longer crashes: `VideoOrImage` is a client component (it had `onLoad` on a server page). Statistics modal uses 7d/30d/90d pills, plots snapshot/live series without dummy waves, and gives short links a finished empty state. Switching channels resets the right pane scroll, not the rail. i18n +3 (`top_post`, `top_n_posts`, `no_short_link_results_hint`). `scripts/ui-migration-check.sh --update` wrote `i18n.txt`.

**Analytics All channels layout, tooltip, and post actions.** Live All channels was cramped by the 38/62 Mix+Top 5 split (fifth card wrapped) and a numeric All-row badge. Top 5 is now a full-width row of equal cards with horizontal snap-scroll below ~1100px. By channel, Posting days, and Engagement mix sit in an equal trio (xl 3 / md 2+1 / phone stack). ChannelMix caps at top 6 + Other (`plus_n_channels`) and hides 0 impressions. Engagement mix shows when any reactions or comments are known (Facebook unknown comments no longer hide the widget). All channels rail and phone sheet use a 4-icon mosaic + `n_channels`. Chart tooltips sit above the hovered point; spark labels stay one day. Table, Top 5, and phone rows reuse calendar Preview / Duplicate / Delete plus enlarge and go to post. i18n +4 (`post_actions`, `enlarge_image`, `go_to_post`, `plus_n_channels`); `all_channels_hint` unused. `scripts/ui-migration-check.sh --update` wrote `i18n.txt`.
...
**MCP analytics + phone dashboard.** Agent/MCP gained `analyticsSummaryTool`, `analyticsPostsTool` and `analyticsPostTool` (16 registry tools + `ask_postqueen` = 17). `/analytics` on phone shows All channels immediately behind a channel sheet instead of a closed drawer. Performance per post stacks on phone and scrolls sideways on tablet. i18n +2 (`choose_channel`, `refreshing_analytics`). `scripts/ui-migration-check.sh --update` wrote `i18n.txt`.

**All-channels post analytics.** `/analytics` defaults to All channels when more than one reporting network is connected. Summary KPIs, Top 5 posts (thumbnail left) and a Performance per post table read `GET /analytics/summary` and `GET /analytics/posts`. Lifetime totals, honest dashes for Facebook comments / Pinterest reactions. i18n keys added for the new copy. `scripts/ui-migration-check.sh --update` wrote `api.txt` and `i18n.txt`.

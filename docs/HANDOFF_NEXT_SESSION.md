...
- ✅ **Client Dashboard** (session 2026-07-26) — per-client `/clients/[id]` (`components/clients/client-dashboard.component.tsx` + `clients/[id]/page.tsx`; Clients rows navigate here). 4 tabs, all real data via EXISTING endpoints (no schema change): **Overview** (active/total accounts, scheduled/published/attention stats, next scheduled post, connected-accounts summary, recent activity), **Accounts** (client's channels + health + real Reconnect), **Content** (`GET /posts/list?customer=<id>&state=scheduled|published|draft` → `expandPostsList`; All/Scheduled/Published/Drafts filter), **Analytics** (DB-DERIVED only — published-per-6-months bar, published-by-channel, counts). **LIVE** (commit `f2564ef7`; verified route + marker in container). **Two deliberate decisions:** (1) **Campaigns tab OMITTED** until the Campaigns module (#6) exists — no fake tab. (2) Analytics uses ONLY our DB (post volume/cadence/status), NOT the live-provider `/analytics/:integration` endpoints, because those refresh OAuth + call the live provider on cache-miss = would TOUCH the Époque IG. Platform reach/engagement can be surfaced per-channel later once the owner clears IG (endpoints exist + the `/analytics` page already uses them).
...
3. **Calendar + Composer premium pass** — restyle the EXISTING (already-functional) calendar/`new-launch` composer/schedule to the new look. Do NOT rebuild working scheduling/publishing. **← NEXT (owner says "go on")**

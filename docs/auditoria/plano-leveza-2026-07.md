...
|---|---|---|
| C1 | `@copilotkit/*`, `mastra`, `@mastra/*`, `@langchain/*`, `@ag-ui/mastra`, `openai` | **Tentativa de finalização 2026-07-03 — parcialmente feita, bloqueio real encontrado.** `@copilotkit/*`/`@langchain/*`/`openai` confirmados de novo como core (editor/autopost/media/video) — não remover. **`mastra`/`@mastra/*`/`@ag-ui/mastra`: NÃO são seguros de remover, e o motivo original estava incompleto.** `copilot.controller.ts` tem 2 rotas Mastra-específicas (`/agent`, `/list`, `/:thread/list`) que seriam removíveis, MAS o `startMcp()` em `main.ts` (que também usa Mastra) registra o endpoint público **`/mcp/:apiKey`**, documentado na página de API pública (`public.component.tsx`, var `mcpUrl`) para ferramentas de IA externas (ex. Claude Desktop) — **não é só o backend do chat `/agents`**. Sem dado de uso real desse endpoint MCP público (diferente do Farcaster/Nostr, onde a query em produção confirmou zero uso), a remoção do backend foi **revertida por completo** (código + deps) depois de detectar isso — só o build real pegou, porque o `tsc` sozinho não sabe que uma rota documentada publicamente "importa". Removida com segurança só a **UI standalone da página `/agents`** (chat, já invisível por flag desde a Fase B1, consumia só a rota `/copilot/agent` que continua existindo). **Import morto do `MastraService` em `auth.middleware.ts`** (achado da rodada anterior) segue sem tocar — seria removido junto se um dia o C1 avançar, mas por ora fica (não vale isolar). **Decisão pendente do Felipe:** rodar uma consulta/checagem de uso real do endpoint `/mcp/:apiKey` (logs de acesso, analytics, ou perguntar direto se algum cliente/parceiro usa) antes de reconsiderar remover Mastra — sem isso, C1 fica **definitivamente bloqueado**, não é "adiado por preguiça". |
...
| C4 | Pinar o que sobrar de IA/volátil (VOC-41) | Ainda não feito — avaliar numa próxima rodada (deps de IA como `@mastra/*`/`@langchain/*` continuam em uso real, ver C1; não há "sobra" ainda pra pinar). |

...
| C3 | Web3/nicho: `@solana/*`, `viem`, `bs58`, `tweetnacl`, `nostr-tools`, `@neynar/*`, `@postiz/wallets` | **B3 rodado em produção 2026-07-03 (Felipe, via Supabase SQL Editor):** `SELECT count(*) total, count(*) FILTER (WHERE "deletedAt" IS NULL) ativas FROM "Integration";` → **`total=0, ativas=0`**. A tabela `Integration` está **completamente vazia** — nenhum cliente conectou nenhum canal ainda (não é só web3/nicho; é literalmente zero de qualquer provider). Isso **desbloqueia C3 tecnicamente**, mas remover Farcaster/Nostr/wallet-login não é só "podar dependência não usada" — é **desligar métodos de login/postagem inteiros do produto** (ainda alcançáveis por UI mesmo sem uso real hoje). Tratar como decisão de produto explícita do Felipe antes de executar, não como limpeza automática. Se aprovado: remover `libraries/nestjs-libraries/src/integrations/social/{farcaster,nostr}.provider.ts`, `apps/backend/src/services/auth/providers/{wallet,farcaster}.provider.ts`, `apps/frontend/src/components/auth/providers/{wallet,farcaster}.provider.tsx`, `apps/frontend/src/components/launches/web3/providers/*`, `apps/frontend/src/components/auth/nayner.auth.button.tsx` + as deps `@solana/*`, `viem`, `bs58`, `tweetnacl`, `nostr-tools`, `@neynar/*`, `@postiz/wallets` do `package.json`, com `pnpm install` real + build/boot antes de commitar (só possível a partir de `C:\dev\vocaccio` com rede normal — esta sessão tem egress restrito, confirmado ao tentar o boot do editor Konva). |
